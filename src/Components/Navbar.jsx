...
export default function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/80 dark:bg-gray-900/95 backdrop-blur-xl border-b border-gray-200 dark:border-gray-800 transition-all duration-300 supports-[backdrop-filter]:bg-white/60 dark:supports-[backdrop-filter]:bg-gray-900/90">
      <div className="container mx-auto px-4 sm:px-6 max-w-7xl">
        <div className="flex justify-between items-center h-16">
          ...
          <Link to="/" className="flex items-center gap-2 group shrink-0" onClick={triggerLogoAnimation}>
            ...
            <div className="flex flex-col">
              <span className="text-xl font-bold tracking-tight text-slate-900 dark:text-white bg-linear-to-r from-violet-600 via-fuchsia-600 to-pink-600 dark:from-violet-400 dark:via-fuchsia-400 dark:to-pink-400 bg-clip-text text-transparent">
                MindShift
              </span>
            ...
            {navLinks.slice(0, 5).map((link) => (
              link.path === '/crisis' ? (
                <button
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-300 font-medium text-black dark:text-gray-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-blue-600 dark:hover:text-blue-400 cursor-pointer`}
                >
                ...
                <Link
                  ...
                  className={`flex items-center gap-1.5 px-3 py-2 rounded-lg transition-all duration-300 font-medium ${isActive(link.path)
                    ? 'bg-slate-100 dark:bg-white/10 text-blue-600 dark:text-white shadow-xs'
                    : 'text-slate-600 dark:text-gray-300 hover:bg-slate-100 dark:hover:bg-white/10 hover:text-blue-600 dark:hover:text-blue-400'
                    }`}
                >
                  <span className="text-lg">{link.icon}</span>
                  <span className="text-sm whitespace-nowrap">{link.name}</span>
        ...
        {isMenuOpen && (
          <div className="lg:hidden border-t border-gray-200 dark:border-gray-800 py-3 animate-slideDown bg-white/98 dark:bg-gray-900/98 backdrop-blur-xl">
            <div className="space-y-1">
              {navLinks.map((link) => (
                <Link
                  ...
                  className={`flex items-center gap-3 px-4 py-2.5 rounded-lg transition-colors ${isActive(link.path)
                    ? 'bg-slate-100 dark:bg-white/10 text-blue-600 dark:text-white'
              ...
              <div className="flex gap-2 pt-3 mt-2 border-t border-slate-200 dark:border-gray-800 px-4">
                <button
                  onClick={toggleTheme}
                  className="flex-1 inline-flex items-center justify-center font-medium rounded-lg transition-all duration-300 focus:outline-none focus:ring-2 focus:ring-offset-2 active:scale-95 cursor-pointer gap-2 p-2.5 bg-slate-100 dark:bg-gray-800 hover:bg-slate-200 dark:hover:bg-gray-700 text-slate-700 dark:text-gray-300"
                >
                  {theme === 'light' ? (
                    <>
